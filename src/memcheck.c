...
void memcheck_metrics_add_range(
    memcheck_metrics_t *metrics, poiter_range_t range
) {
...
void memcheck_metrics_report_print(
    uart_t *uart,
    char *report_name,
...
#define print(str) uart_puts(uart, str)
    ...
    if (metrics->total_ranges_found != 0) {
        ...
        for (size_t ranges_counter = 0; ranges_counter < metrics->nranges;) {
            ...
            for (int i = 0;
                 i < ranges_per_line && ranges_counter < metrics->nranges;
                 ++i) {
                buf[idx++] = ' ';
        ...
        if (metrics->total_ranges_found > metrics->nranges) {
            ...
            int idx = 0;
            uint_to_string(
                buf, &idx, metrics->total_ranges_found - metrics->nranges
            );
            print("  and ");
            print(buf);
            print(" more ranges.\n");
...
int fill_ram(
    ...
    for (int i = 0; i < nchannels; i++) {
        ...
        tmp->next_config.write_ptr =
            (uint64_t)start + i * channel_offset * chunk_size;
    ...
    if (chunks_to_fill % nchannels != 0) {
        chunks_for_channel[nchannels - 1] +=
            chunks_to_fill - (chunks_to_fill * nchannels);
    }
    ...
    while (chunks_for_channel[0] || chunks_for_channel[1]
           || chunks_for_channel[2] || chunks_for_channel[3]) {
        for (int i = 0; i < nchannels; ++i) {
            ...
            if (channels[i].control.error == 1) {
                if (channels_tmp[i] == 0) {
                    memcheck_metrics_add_range(
                        metrics,
                        (poiter_range_t){
                            .from = (void *)channels[i].next_config.write_ptr
                                  - chunk_size,
                    ...
                    memcheck_metrics_last_range(metrics).to =
                        (void *)channels[i].next_config.write_ptr;
                }
            } else {
                channels_tmp[i] = 0;
            }
